...
const Navbar = () => {
  ...
  return (
    <div className='text-white max-w-screen-2xl container mx-auto px-4 md:px-20 shadow-md h-16 fixed top-0 right-0 left-0  bg-[#28293e]'>
       <div className='flex justify-between items-center h-16 z-[1]' >
          ...
          <ul className=' hidden md:flex space-x-30'>
             {navItems.map((item) => (
              <li key={item.id} className='hover:scale-105 duration-200 cursor-pointer text-[17px] font-thin'> {item.name}</li>
             ))}
          </ul>
          ...
          <div className='flex space-x-2'>
            ...
            <h1 className='text-[14px]  font-bold  px-6 py-2' style={{fontFamily : "sans-serif" , border : "1px solid white" , borderRadius : "5px"}}>CONTACTS</h1>
          </div>
